...
with main_container:
    ...
    if audio:
        ...
        with anal_col2:
            # [추가] 수동 구간 조절 기능
            st.info("💡 녹음된 파형을 확인하고, 실제 목소리가 담긴 구간만 선택하세요.")
            audio_stream_init = io.BytesIO(audio['bytes'])
            full_audio_init = AudioSegment.from_file(audio_stream_init)
            duration_sec = len(full_audio_init) / 1000.0
            
            # 시간 범위 선택 슬라이더
            time_range = st.slider("분석할 시간 범위를 선택하세요 (초):", 
                                   0.0, duration_sec, (0.0, duration_sec), step=0.1)
            
            st.success("✅ 구간 설정 완료! 아래 버튼을 눌러 분석을 시작하세요.")
...
if st.session_state.get('analysis_done'):
    try:
        ...
        with tab3:
            st.info("✂️ 선택하신 구간의 파형입니다 (비프음 등 잡음 제외)")
            ...
            ax2.set_title("Your Cropped Waveform")
        ...
        with tab4:
            ...
            ax_n.set_title("Native Intonation"); ax_l.set_title("Your Intonation")
            st.pyplot(fig2)

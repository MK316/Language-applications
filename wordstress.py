...
if audio:
    ...
    with open("temp_raw.wav", "wb") as f: f.write(audio['bytes'])
...
if st.session_state.get('analysis_done'):
    with st.spinner("리듬 및 에너지 대조 중..."):
        try:
            ...
            tts = gTTS(text=target_word, lang='en'); n_fp = io.BytesIO(); tts.write_to_fp(n_fp); n_fp.seek(0)
            n_seg = AudioSegment.from_file(n_fp); ns, ne = get_speech_bounds(n_seg)
            ...
            with tab1:
                ...
                ax1.set_title("My Energy Profile (Waveform + Envelope)"); ax2.set_title("Native Standard Profile")
